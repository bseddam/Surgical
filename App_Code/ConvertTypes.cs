...
public static class ConvertTypes
{
    ...
    public static object ToParseDatetime(this string value)
    {
        DateTime datevalue;
        object datecon;
        if (DateTime.TryParse(value, out datevalue))
        {
            datecon = DateTime.Parse(value.Trim()).ToString("yyyy-MM-dd");

        }
        else
        {
            datecon = DBNull.Value;
        }

        return datecon;
    }

    public static object ToParseFloat(this string value)
    {
        float datevalue;
        object datecon;
        if (float.TryParse(value, out datevalue))
        {
            datecon = float.Parse(value.Trim());
        }
        else
        {
            datecon = DBNull.Value;
